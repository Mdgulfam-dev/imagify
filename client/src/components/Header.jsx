...
const Header = () => {
  ...
  return (
    ...
    >
      ...
      <motion.button onClick={onClickHandler} className='sm:tet-lg text-white bg-black w-auto mt-8 px-12 py-2.5 flex items-center gap-2 rounded-full'
      whileHover={{scale: 1.05}}
      whileTap={{scale: 0.95}}
      initial={{opacity:0}}
      animate={{opacity: 1}}
      transition={{default: {default: 0.5}, opacity: {delay: 0.8, duration: 1}}}
      ...
      >
        {Array(6).fill('').map((item, index)=>(
            ...
            className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' src={assets.sample_img_1} alt='' key={index} width={70} />
        ))}
      </motion.div>
      <motion.p
      initial={{opacity: 0}}
